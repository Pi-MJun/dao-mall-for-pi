...
class Hook
{
    ...
    public function html($params = [])
    {
        ...
        if($ret['code'] == 0)
        {
            return MyView('../../../plugins/view/expressforkdn/index/public/content', [
                'data'  => $ret['data'],
            ]);
        } else {
            return $ret['msg'];
    ...
    public function operation($params = [])
    {
        ...
        {
            $style = ($params['hook_name'] == 'plugins_view_index_order_detail_operate') ? '' : 'am-btn-block';
            return '<button type="button" class="am-btn am-btn-warning am-btn-xs am-radius '.$style.' plugins-expressforkdn-submit" data-exp-id="'.$params['data']['express_id'].'" data-exp-num="'.$params['data']['express_number'].'"><i class="am-icon-cube"></i> <span>物流</span></button>';
        }
        return '';
    }
}
