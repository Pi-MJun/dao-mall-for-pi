...
class Goods
{
    ...
    public function Run($params = [])
    {
        ...
        $data = [
            ...
            'base' => [
                ...
                'is_search'     => 1,
                'search_url'    => PluginsHomeUrl('shop', 'goods', 'index'),
                'is_delete'     => 1,
                'delete_url'    => PluginsHomeUrl('shop', 'goods', 'delete'),
                'delete_key'    => 'ids',
                'detail_title'  => '基础信息',
                'is_middle'     => 0,
            ],
            ...
            'form' => [
                ...
                [
                    'label'         => '商品信息',
                    'view_type'     => 'module',
                    'view_key'      => '../../../plugins/view/shop/index/goods/module/info',
                    'grid_size'     => 'lg',
                    'is_sort'       => 1,
                    'sort_field'    => 'title',
                    'search_config' => [
                ...
                [
                    ...
                    'search_config' => [
                        ...
                        'data'                  => GoodsService::GoodsCategory(['is_all'=>1]),
                    ],
                ],
                [
                    'label'         => '店铺商品分类',
                    'view_type'     => 'field',
                    'view_key'      => 'shop_category_id',
                    'view_data'     => array_column($this->ShopGoodsCategoryList(), 'name', 'id'),
                    'is_sort'       => 1,
                    'search_config' => [
                        'form_type'         => 'select',
                        'where_type'        => 'in',
                        'data'              => $this->ShopGoodsCategoryList(),
                        'data_key'          => 'id',
                        'data_name'         => 'name',
                        'is_multiple'       => 1,
                    ],
                ...
                [
                    'label'         => '操作',
                    'view_type'     => 'operate',
                    'view_key'      => '../../../plugins/view/shop/index/goods/module/operate',
                    'align'         => 'center',
                    'fixed'         => 'right',
                ],
            ],
        ...
        // 是否开启商家商品上下架和库存扣除开关操作
        $item = BaseService::GoodsFormStatusData();
        if(!empty($item))
        {
            array_splice($data['form'], 9, 0, $item);
        }

        // 用户是否开启商品结算设置
        $shop = ShopService::UserShopInfo($this->user_id, 'user_id', 'is_user_settle');
        if(!empty($shop) && isset($shop['is_user_settle']) && $shop['is_user_settle'] == 1)
        {
            array_splice($data['form'], -3, 0, BaseService::GoodsFormSettleData());
        }
     ...
     * @desc    description
     */
    public function ShopGoodsCategoryList()
    {
        $ret = ShopGoodsCategoryService::GoodsCategoryAll(['user_id'=>$this->user_id, 'field'=>'id,name']);
