...
class SlideService
{
     ...
     */
    public static function ClientSlideList($params = [])
    {
        // 获取数据
        $data = Db::name('PluginsBlogSlide')->field('name,images_url,event_value,event_type,bg_color')->where(['platform'=>APPLICATION_CLIENT_TYPE, 'is_enable'=>1])->order('sort asc,id asc')->select()->toArray();
        if(!empty($data))
        {
            foreach($data as &$v)
            {
    ...
    public static function SlideList($params = [])
    {
        ...
        $data = Db::name('PluginsBlogSlide')->field($field)->where($where)->order($order_by)->limit($m, $n)->select()->toArray();
        if(!empty($data))
        {
    ...
    {
        return (int) Db::name('PluginsBlogSlide')->where($where)->count();
    }
    ...
    public static function SlideSave($params = [])
    {
        ...
        {
            $data['add_time'] = time();
            if(Db::name('PluginsBlogSlide')->insertGetId($data) > 0)
            {
            ...
            $data['upd_time'] = time();
            if(Db::name('PluginsBlogSlide')->where(['id'=>intval($params['id'])])->update($data))
            {
    ...
    public static function SlideDelete($params = [])
    {
        ...
        // 删除操作
        if(Db::name('PluginsBlogSlide')->where(['id'=>$params['ids']])->delete())
        {
    ...
    public static function SlideStatusUpdate($params = [])
    {
        ...
        // 数据更新
        if(Db::name('PluginsBlogSlide')->where(['id'=>intval($params['id'])])->update([$params['field']=>intval($params['state']), 'upd_time'=>time()]))
        {
